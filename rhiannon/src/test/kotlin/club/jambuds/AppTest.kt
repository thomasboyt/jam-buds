...
open class AppTest {
    ...
    fun wire(txn: Handle) {
        ...
        app = Application.createJavalinApp(false)
    ...
    companion object {
        val config = Application.getConfig()
        val jdbi = Application.createJdbi(config.getString("databaseUrl"))
        val redis = RedisClient.create(config.getString("redisUrl")).connect()!!
        val objectMapper = Application.createObjectMapper()
