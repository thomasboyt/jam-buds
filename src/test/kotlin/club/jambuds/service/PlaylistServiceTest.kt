...
class PlaylistServiceTest : TransactionTest() {
    ...
    @Test
    fun `getPublicFeed - aggregates posts of the same song and sets timestamp to oldest post`() = withTransaction { txn ->
        val playlistService = createPlaylistService(txn)
    ...
    @Test
    fun `getUserFeed - only includes posts from users the current user follows and their own`() = withTransaction { txn ->
        val playlistService = createPlaylistService(txn)
